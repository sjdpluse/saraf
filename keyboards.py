...
def currency_quote_keyboard(code: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[InlineKeyboardButton("🧮 صراف", callback_data=f"curcalc:{code}")]])


def gold_karat_keyboard() -> InlineKeyboardMarkup:
    rows = [[InlineKeyboardButton(f"عیار {k}", callback_data=f"gold:{k}")] for k in GOLD_KARATS]
    rows.append([InlineKeyboardButton("🔄 نمایش همهٔ عیارها", callback_data="gold:all")])
    rows.append([InlineKeyboardButton("🧮 ماشین‌حساب خرید/فروش طلا", callback_data="gold:calc")])
    return InlineKeyboardMarkup(rows)


def gold_calc_mode_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[InlineKeyboardButton("🟢 خرید طلا", callback_data="goldcalc_mode:buy"), InlineKeyboardButton("🔴 فروش طلا", callback_data="goldcalc_mode:sell")]])


def gold_calc_karat_keyboard(mode: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[InlineKeyboardButton(f"عیار {k}", callback_data=f"goldcalc_karat:{mode}:{k}")] for k in GOLD_KARATS])


def silver_calc_mode_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[InlineKeyboardButton("🟢 خرید نقره", callback_data="silvercalc_mode:buy"), InlineKeyboardButton("🔴 فروش نقره", callback_data="silvercalc_mode:sell")]])
...
def compare_period_keyboard(target_code: str, target_name: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("⏱ ۲۴ ساعت پیش", callback_data=f"cmp_period:{target_code}:1"), InlineKeyboardButton("📅 ۷ روز پیش", callback_data=f"cmp_period:{target_code}:7")],
        [InlineKeyboardButton("🗓 ۳۰ روز پیش", callback_data=f"cmp_period:{target_code}:30"), InlineKeyboardButton("📆 ۹۰ روز پیش", callback_data=f"cmp_period:{target_code}:90")],
    ])
...
def usdt_menu_keyboard() -> InlineKeyboardMarkup:
    ...
    if MINI_APP_URL:
        ...
        rows.extend([
            [InlineKeyboardButton("🟢 خرید USDT", callback_data="usdt_action:USDT:buy"), InlineKeyboardButton("🔴 فروش USDT", callback_data="usdt_action:USDT:sell")],
            [InlineKeyboardButton("🔵 خرید USDC", callback_data="usdt_action:USDC:buy"), InlineKeyboardButton("🔷 فروش USDC", callback_data="usdt_action:USDC:sell")],
        ])
    return InlineKeyboardMarkup(rows)


def usdt_continue_keyboard(action: str, asset: str = "USDT") -> InlineKeyboardMarkup:
    asset = str(asset or "USDT").upper()
    verb = "خرید" if action == "buy" else "فروش"
    info_url = _support_chat_url(f"سلام، در مورد خرید و فروش {asset} در Saraf معلومات بیشتر می‌خواهم.")
    support_url = _support_chat_url(f"سلام، برای خرید و فروش {asset} در Saraf به پشتیبانی نیاز دارم.")
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(f"✅ درخواست {verb} {asset}", callback_data=f"usdt_continue:{action}")],
        [InlineKeyboardButton("💬 اطلاعات بیشتر", url=info_url), InlineKeyboardButton("🎧 پشتیبانی", url=support_url)],
    ])


def usdt_payment_method_keyboard(action: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[InlineKeyboardButton("🏢 حضوری", callback_data=f"usdt_pay:{action}:in_person"), InlineKeyboardButton("🏦 آنلاین", callback_data=f"usdt_pay:{action}:online")]])


def usdt_in_person_paid_keyboard(action: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[InlineKeyboardButton("✅ پرداخت را انجام دادم", callback_data=f"usdt_paid:{action}")]])


def usdt_network_keyboard(action: str) -> InlineKeyboardMarkup:
    rows = [[InlineKeyboardButton(net, callback_data=f"usdt_net:{action}:{net}")] for net in USDT_NETWORKS]
...
def usdt_buy_exchange_keyboard() -> InlineKeyboardMarkup:
    ...
    for ex in USDT_EXCHANGES:
        ...
        if len(row) == 2:
            rows.append(row); row = []
    if row: rows.append(row)
    rows.append([InlineKeyboardButton("👛 کیف پول شخصی / دیگر", callback_data="usdt_buy_exch:other")])
    return InlineKeyboardMarkup(rows)


def admin_order_review_keyboard(order_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[InlineKeyboardButton("✅ تایید سفارش", callback_data=f"admin_confirm:{order_id}"), InlineKeyboardButton("❌ رد سفارش", callback_data=f"admin_reject:{order_id}")]])


def admin_order_complete_keyboard(order_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[InlineKeyboardButton("📦 تکمیل شد (پرداخت/واریز انجام شد)", callback_data=f"admin_complete:{order_id}")]])


def usdt_rating_keyboard(order_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[InlineKeyboardButton("⭐" * n, callback_data=f"usdt_rate:{order_id}:{n}") for n in range(1, 6)]])


def kyc_phone_keyboard() -> ReplyKeyboardMarkup:
    return ReplyKeyboardMarkup([[KeyboardButton("📱 ارسال شمارهٔ تماس من", request_contact=True)]], resize_keyboard=True, one_time_keyboard=True)


def kyc_review_keyboard(chat_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[InlineKeyboardButton("✅ تایید هویت", callback_data=f"admin_kyc_verify:{chat_id}"), InlineKeyboardButton("❌ رد هویت", callback_data=f"admin_kyc_reject:{chat_id}")]])
