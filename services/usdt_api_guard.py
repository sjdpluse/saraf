...
from services import webapp_auth, usdt_service
...
def _create_quote(chat_id: int, action: str, amount: float, quote: dict) -> dict:
    ...
    row = db.get_client().table("usdt_quotes").insert({
        "chat_id": chat_id, "order_type": action, "usdt_amount": amount,
    ...
    result = dict(quote)
    result.update({"quote_id": row.data[0]["id"], "expires_at": expires_at.isoformat()})
    return result


def _load_quote(chat_id: int, quote_id: int, action: str, amount: float) -> dict:
    res = (db.get_client().table("usdt_quotes").select("*").eq("id", quote_id)
           .eq("chat_id", chat_id).eq("order_type", action).limit(1).execute())
    if not res.data:
...
async def _guard_quote(x_telegram_init_data: Optional[str] = Header(None)) -> None:
    try:
...
async def _guard_order(
    ...
    _order_context.set({"chat_id": user["id"], "key": key, "quote_id": quote_id, "quote": quote})
...
def _patch_quote_service() -> None:
    ...
    async def guarded_buy(amount):
        ...
        if ctx and ctx["quote"]["order_type"] == "buy":
            q = ctx["quote"]
            base = float(q["usdt_amount"]) * float(q["usd_rate"])
            return {"amount": float(q["usdt_amount"]), "usd_rate": float(q["usd_rate"]),
                    "fee_percent": float(q["fee_percent"] or 0), "base_afn": round(base, 1),
                    "fee_afn": round(float(q["total_afn"]) - base, 1),
                    "total_afn": float(q["total_afn"]), "total_usd": float(q["total_usd"]),
                    "quote_id": q["id"], "expires_at": q["expires_at"]}
        ctx = _quote_context.get()
        quote = await original_buy(amount)
        return _create_quote(ctx["chat_id"], "buy", amount, quote) if ctx else quote
    ...
    async def guarded_sell(amount):
        ...
        return _create_quote(ctx["chat_id"], "sell", amount, quote) if ctx else quote
...
def _patch_order_service() -> None:
    ...
    if getattr(usdt_order_service, "_saraf_order_guard_installed", False):
        return
    original_buy, original_sell = usdt_order_service.create_buy_order, usdt_order_service.create_sell_order

    async def guarded_buy(**kwargs):
        ctx = _order_context.get()
        if not ctx or kwargs.get("source") != "miniapp":
            return await original_buy(**kwargs)
        existing = (db.get_client().table("usdt_orders").select("*").eq("chat_id", ctx["chat_id"])
                    .eq("idempotency_key", ctx["key"]).limit(1).execute())
        if existing.data:
            row = existing.data[0]
            return {"order_id": row["id"], "order_code": usdt_order_service.build_order_code(row["id"]),
                    "message": "سفارش قبلی شما برای همین درخواست موجود است.", "risk_level": row.get("risk_level")}
    ...
    usdt_order_service.create_buy_order, usdt_order_service.create_sell_order = guarded_buy, guarded_sell
    usdt_order_service._saraf_order_guard_installed = True


def _patch_order_insert() -> None:
    original = getattr(db, "insert_usdt_order", None)
    if original is None or getattr(db, "_saraf_order_insert_guard_installed", False):
        return
    def guarded_insert(order: dict):
        ctx = _order_context.get()
        if ctx and order.get("source") == "miniapp":
            order = dict(order)
            order["idempotency_key"] = ctx["key"]
            order["quote_id"] = ctx["quote_id"]
        return original(order)
    db.insert_usdt_order = guarded_insert
    db._saraf_order_insert_guard_installed = True
