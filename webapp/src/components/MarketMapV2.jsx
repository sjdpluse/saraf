...
function provincePathFor(provincePaths, location) {
  const aliases = PROVINCE_NAME_MAP[location] || [];
  const sourceName = aliases.find((name) => provincePaths[name]);
  return sourceName ? provincePaths[sourceName] : null;
}

function pointToSvg(location) {
  const point = PROVINCE_POINTS[location] || PROVINCE_POINTS["کابل"];
  return [point[0] * 10, point[1] * (666.6667 / 100)];
}

function ProvinceOverlay({ activeLocation }) {
  const [provincePaths, setProvincePaths] = useState({});
  const currentLocationRef = useRef(activeLocation);
  const [transition, setTransition] = useState({ from: null, to: activeLocation, id: 0 });
  ...
  useEffect(() => {
    if (activeLocation === currentLocationRef.current) return undefined;
    const from = currentLocationRef.current;
    currentLocationRef.current = activeLocation;
    let nextId = 0;
    setTransition((current) => {
      nextId = current.id + 1;
      return { from, to: activeLocation, id: nextId };
    });
    const timer = window.setTimeout(() => {
      setTransition((current) => current.id === nextId ? { ...current, from: null } : current);
    }, 980);
    return () => window.clearTimeout(timer);
  }, [activeLocation]);

  const toPath = provincePathFor(provincePaths, transition.to);
  const fromPath = transition.from ? provincePathFor(provincePaths, transition.from) : null;
  if (!toPath && !fromPath) return null;

  const fromPoint = transition.from ? pointToSvg(transition.from) : null;
  const toPoint = pointToSvg(transition.to);
  const fromClipId = `province-from-${transition.id}`;
  const toClipId = `province-to-${transition.id}`;

  return (
    <svg className="province-boundary-overlay province-hourglass-overlay" viewBox="0 0 1000 666.6667" preserveAspectRatio="none" aria-hidden="true">
      <defs>
        {fromPath && <clipPath id={fromClipId}><path d={fromPath} /></clipPath>}
        {toPath && <clipPath id={toClipId}><path d={toPath} /></clipPath>}
        <filter id="province-dark-filter" colorInterpolationFilters="sRGB">
          <feColorMatrix type="matrix" values="0 0 0 0 0.0863  0 0 0 0 0.0863  0 0 0 0 0.0863  0 0 0 1 0" />
        </filter>
      </defs>
      ...
      {fromPath && fromPoint && (
        <g
          ...
          className="province-hourglass-layer province-hourglass-out"
          style={{ transformOrigin: `${fromPoint[0]}px ${fromPoint[1]}px` }}
        >
          <image
            href={afghanistanMap}
            x="0" y="0" width="1000" height="666.6667"
            preserveAspectRatio="none"
            clipPath={`url(#${fromClipId})`}
            filter="url(#province-dark-filter)"
            className="province-dot-image"
          />
        </g>
      )}

      {fromPath && fromPoint && toPath && (
        <path
          key={`sand-${transition.id}`}
          className="province-sand-stream"
          d={`M${fromPoint[0]},${fromPoint[1]} L${toPoint[0]},${toPoint[1]}`}
          pathLength="1"
        />
      )}
      ...
      {toPath && (
        <g
          key={`in-${transition.id}`}
          className={`province-hourglass-layer ${fromPath ? "province-hourglass-in" : "province-hourglass-steady"}`}
          style={{ transformOrigin: `${toPoint[0]}px ${toPoint[1]}px` }}
        >
          <image
            href={afghanistanMap}
            x="0" y="0" width="1000" height="666.6667"
            preserveAspectRatio="none"
            clipPath={`url(#${toClipId})`}
            filter="url(#province-dark-filter)"
            className="province-dot-image"
          />
        </g>
...
export default function MarketMapV2({ activeLocation, activeDuration = 1700 }) {
