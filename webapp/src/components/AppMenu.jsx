  ...
  return <>
    ...
    >
      <div className="menu-panel">
        <header>
          <h2 id="menu-title">منوی صراف</h2>
          <button className="icon-button" aria-label="بستن منو" onClick={() => setOpen(false)}><X size={21} /></button>
        </header>
        <nav aria-label="منوی صراف">
          {LINKS.map(([page, label, Icon]) => (
            <button key={page} onClick={() => go(page)}>
              <Icon size={21} /><span>{label}</span><CaretLeft size={17} />
            </button>
          ))}
        </nav>
        <button className="menu-support" onClick={() => { setOpen(false); openTelegramChat("SJDPLUS"); }}>
          <Headset size={23} /><span>پشتیبانی صراف<small>گفتگو در تلگرام</small></span><CaretLeft size={17} />
        </button>
      </div>
    </dialog>
  </>;
