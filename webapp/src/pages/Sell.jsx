    ...
    <div className="app-shell">
      ...
      {step === "quote" && quote && (
        <div className="card animate-in">
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 16 }}><button className="btn btn-sell" onClick={() => checkGateAndProceed(parseFloat(amount), quote)} disabled={checkingProfile}>{checkingProfile ? <span className="spinner" /> : <>ادامهٔ درخواست فروش <ArrowRight size={16} weight="bold" /></>}</button><div className="help-actions-grid"><button className="btn btn-outline" onClick={() => openTelegramChat("SJDPLUS", `سلام، در مورد خرید و فروش ${selectedAsset} در صراف معلومات بیشتر می‌خواهم.`)}><ChatCircleDots size={17} /> اطلاعات بیشتر</button><button className="btn btn-outline" onClick={() => openTelegramChat("SJDPLUS", `سلام، برای خرید و فروش ${selectedAsset} در صراف به پشتیبانی نیاز دارم.`)}><Headset size={17} /> پشتیبانی</button><WhatsAppActionButton mode="support" asset={selectedAsset} /></div></div>
        </div>
      )}
