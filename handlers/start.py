...
WELCOME_TEXT = (
    ...
    "📊 مقایسه با گذشته  |  🔄 مبدل ارز  |  🪙 خرید و فروش USDT / USDC\n\n"
...
ABOUT_HOME_TEXT = (
    ...
    "خرید و فروش USDT و USDC را برای کاربران افغانستان ساده‌تر، شفاف‌تر و قابل پیگیری‌تر کند.\n\n"
...
ABOUT_USDT_TEXT = (
    ...
    "صراف روند خرید و فروش تتر (USDT) و USD Coin (USDC) را از دریافت نرخ تا ثبت و پیگیری سفارش "
    "در یک مسیر روشن و قابل فهم فراهم می‌کند.\n\n"
    f"• محدودهٔ سفارش هر دارایی: *{USDT_MIN_AMOUNT:g} تا {USDT_MAX_AMOUNT:g} واحد*\n"
    "• انتخاب USDT یا USDC پیش از معامله\n"
    ...
    "• کد اختصاصی برای هر سفارش با نماد دارایی\n"
    "• پیگیری وضعیت سفارش تا تکمیل\n"
    "• مشاهدهٔ سفارش‌های قبلی و ثبت امتیاز پس از معامله\n\n"
    "🚀 خدمات USDT / USDC از طریق مینی‌اپ تلگرام صراف در دسترس است."
)
...
ABOUT_SECURITY_TEXT = (
    ...
    f"• تایید هویت تکمیلی برای سفارش‌های بالاتر از *{USDT_IDENTITY_VERIFICATION_THRESHOLD_USD:g} دالر*\n"
    "• بررسی دستی سفارش‌های مالی پیش از تکمیل\n"
    "• کنترل نوع دارایی، معلومات پرداخت، شبکه و آدرس مقصد\n"
...
DISCLAIMER_TEXT = (
    ...
    "• نرخ‌های عمومی صراف برای اطلاع‌رسانی و محاسبه ارائه می‌شوند و لزوماً قیمت قطعی معامله در بازار خارج از صراف نیستند.\n"
    "• معلومات بازار به‌معنای توصیهٔ سرمایه‌گذاری یا تضمین سود نیست.\n"
    "• پیش از تایید هر معاملهٔ USDT یا USDC، نوع دارایی، مبلغ، نرخ، کارمزد، شبکه و آدرس مقصد را بررسی کنید.\n"
    "• مسئولیت صحت نوع دارایی، آدرس کیف پول و معلوماتی که کاربر وارد می‌کند بر عهدهٔ خود کاربر است.\n"
...
def about_keyboard(page: str = "home") -> InlineKeyboardMarkup:
    if page == "home":
        return InlineKeyboardMarkup([
            [InlineKeyboardButton("📊 بازارها و نرخ‌ها", callback_data="about:markets"), InlineKeyboardButton("🧮 ابزارهای مالی", callback_data="about:tools")],
            [InlineKeyboardButton("🪙 خدمات USDT / USDC", callback_data="about:usdt"), InlineKeyboardButton("🛡 امنیت و تایید هویت", callback_data="about:security")],
            [InlineKeyboardButton("👨‍💻 توسعه و برند", callback_data="about:developer"), InlineKeyboardButton("⚠️ شرایط استفاده", callback_data="about:disclaimer")],
        ])
    rows = []
    if page == "usdt":
        rows.append([InlineKeyboardButton("🚀 ورود به خدمات USDT / USDC", callback_data="about:open_usdt")])
    rows.append([InlineKeyboardButton("🔙 بازگشت به درباره صراف", callback_data="about:home")])
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(WELCOME_TEXT.format(name=safe_name), parse_mode="Markdown", reply_markup=main_menu(is_admin=user.id in ADMIN_CHAT_IDS))


async def about(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_text(ABOUT_HOME_TEXT, parse_mode="Markdown", reply_markup=about_keyboard())
...
async def about_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action == "open_usdt":
        await context.bot.send_message(
            ...
            text="🪙 *خرید و فروش USDT / USDC*\n\nدارایی و نوع معامله را انتخاب کنید:",
            parse_mode="Markdown",
...
async def stop(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text("شما از دریافت پیام‌های همگانی ربات خارج شدید. هر وقت خواستید با /start دوباره فعال شوید.")
